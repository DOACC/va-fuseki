...
do_start()
{
    ...
    if $SUPERVISORCTL status | grep -q "unix:///var/run/supervisor.sock no such file"; then
        $SUPERVISORD
        echo "supervisord started successfully"
        return 0
    fi

    if $SUPERVISORCTL status | grep -qv "unix:///var/run/supervisor.sock no such file"; then
